...
export default function MainScreen() {
  const { builder, patchBuilder, startJob, addToQueue, resetBuilder, jobs, navigate } = useCigs();
  ...
  const recent = jobs.find((j) => j.state === "completed");
    ...
    <div className="cigs-scroll h-full overflow-y-auto">
      <div className="mx-auto max-w-[1180px] p-6">
        <div className="mb-6">
          ...
          <p className="mt-1 text-sm text-muted-foreground">Point Cigs at a source, pick a branch, send it.</p>
        </div>

        <div className="grid grid-cols-1 items-start gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(0,420px)]">
          {/* LEFT — the command HUD */}
          <div className="flex flex-col items-center gap-5">
            ...
            <div className="w-full max-w-[420px] rounded-xl border border-border bg-card p-4">
              ...
              {builder.sourceType === "url" ? (
                ...
                <div className="flex items-center gap-2">
                  <Button data-testid="source-file-pick-button" variant="outline" onClick={pickFile} className="h-10 gap-2">
            ...
            <Hud
              ...
              onSelect={(b) => patchBuilder({ branch: b })}
              runningJob={running}
              hint={builder.branch ? null : sourceOk ? "tap a node" : "add a source"}
            />

            {/* deep-link rails */}
            <div className="grid w-full max-w-[400px] grid-cols-1 gap-2 sm:grid-cols-3">
              <Rail
                testid="rail-now-running"
                tone="live"
                icon={Activity}
                label="Now running"
                value={running ? `${Math.round(running.progress)}% · ${truncate(running.title, 14)}` : "Nothing running"}
                onClick={() => navigate("progress", running?.id)}
              />
              <Rail
                testid="rail-queue"
                tone="warning"
                icon={ListChecks}
                label="Queue"
                value={pendingCount ? `${pendingCount} waiting` : "Empty"}
              ...
              <Rail
                ...
                icon={PackageCheck}
                label="Recent"
                value={recent ? truncate(recent.title, 16) : "No results yet"}
                onClick={() => navigate("result", recent?.id)}
              />
            </div>
          </div>

          {/* RIGHT — the decision path, revealed step by step */}
          <div className="space-y-4">
            ...
            <div data-testid="job-builder-breadcrumb" className="flex flex-wrap items-center gap-1.5 rounded-xl border border-border bg-card px-3 py-2.5">
              <Crumb active={sourceOk} label={sourceOk ? truncate(builder.sourceType === "url" ? builder.url : builder.fileName, 18) : "Source"} />
              <Sep />
            ...
            <AnimatePresence mode="popLayout">
              ...
              {builder.branch ? (
                      ...
                      return (
                        <label
                          ...
                          className={cn(
                            ...
                            active ? "border-primary/55 bg-primary/10" : "border-border hover:border-border hover:bg-surface-2"
                          )}
                        >
                ...
                <motion.div key="target-empty" {...reveal} className="rounded-xl border border-dashed border-border bg-card/50 p-6 text-center">
                  <p className="text-sm text-muted-foreground">Pick a branch on the hub to reveal extraction targets.</p>
                </motion.div>
              )}
                ...
                <motion.div key="confirm" layout {...reveal} className="rounded-xl border border-border bg-card p-4">
                  ...
                  <div className="flex flex-col gap-2 sm:flex-row">
                    <Button
                      ...
                      onClick={startJob}
                      className="h-10 flex-1 gap-2 bg-grad-primary text-primary-foreground transition-opacity hover:opacity-90"
                    >
...
const Rail = ({ testid, tone, icon: Icon, label, value, onClick }) => (
  <button
    data-testid={testid}
    onClick={onClick}
    className="group flex items-center gap-2.5 rounded-xl border border-border bg-card p-3 text-left transition-colors hover:bg-surface-2"
  >
    ...
    <span className="min-w-0">
      ...
      <span className="block truncate text-[12px] font-medium text-foreground">{value}</span>
    </span>
  </button>
);
